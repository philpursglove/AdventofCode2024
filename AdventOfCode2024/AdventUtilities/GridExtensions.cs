...
public static class GridExtensions
{
    ...
    public static List<string> FindDistinctEntries(this string[,] grid, List<string> ignore)
    {
        ...
        for (int y = 0; y < grid.GetUpperBound(1) + 1; y++)
        {
            for (int x = 0; x < grid.GetUpperBound(0) + 1; x++)
            {
                if (!ignore.Contains(grid[x, y]))
                {
                    if (!entries.Contains(grid[x, y]))
                    {
    ...
    public static List<Coordinate> FindEntries(this string[,] grid, string search)
    {
        List<Coordinate> entries = new List<Coordinate>();
        for (int y = 0; y < grid.GetUpperBound(1) + 1; y++)
        {
            for (int x = 0; x < grid.GetUpperBound(0) + 1; x++)
            {
